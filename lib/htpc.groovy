...
def refreshPlexLibrary(server, port = 32400, token = null) {
	tryLogCatch {
		def url = "http://$server:$port/library/sections"
		if (token) {
		...
		if (renameLog){
			...
			def dom = WebRequest.getDocument(url.toURL().text)
			net.filebot.util.XPathUtilities.selectNodes("MediaContainer/Directory", dom).each{ dir ->
				def id = net.filebot.util.XPathUtilities.selectString("@key", dir)
				net.filebot.util.XPathUtilities.selectNodes("Location", dir).each{ loc ->
					def path = net.filebot.util.XPathUtilities.selectString("@path", loc)
					renameLog.each{
						if (it.contains(path)){
							idList.add(id)
						}
		...
		idList.unique().each{
			url = "http://$server:$port/library/sections/$it/refresh"
			if (token) {
				url += "?X-Plex-Token=$token"
			}
			println url
...
def refreshCouchPotatoLibrary(server, port = 5050, path = '', token = null) {
	tryLogCatch {
		println 'Notifying Couch Potato'
		...
		if (renameLog){
			...
			def url = new URL("http://" + server + ':' + port + path + "/api/" + token + "/settings/");
			println url.toString()
			def request = WebRequest.fetch(url,0,null,null)
			def json = (new groovy.json.JsonSlurper()).parseText(java.nio.charset.Charset.forName("UTF-8").decode(request).toString());
			refresh = json.values.manage.library.split('::').any { lib ->
				return renameLog.any{
					if (it.contains(lib)){
						return true
					}
					return false
		...
		if (refresh) {
			def url = new URL("http://" + server + ':' + port + path + "/api/" + token + "/manage.update/?full=0");
			println url.toString()
